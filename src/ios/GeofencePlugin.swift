...
class GeoNotificationManager: NSObject, CLLocationManagerDelegate {
    ...
    func notifyAbout(_ geo: JSON) {
        ...
        //Verifica se a data inicial e a data final da fence não são nill (nulos)
        guard geo["notification"]["data"]["iniDate"] != nil && geo["notification"]["data"]["endDate"] != nil
            else { return }
        
        //Data inicial da fence
        let notInitialDate = getDate(isoDate: geo["notification"]["data"]["iniDate"].stringValue)
        //Data de expiração da fence
        let notEndDate = getDate(isoDate: geo["notification"]["data"]["endDate"].stringValue)

        //Data final da campanha for > hoje, continua para disparar notificação
        guard notEndDate >= Date() else {
            //Se não, campanha finalizada, remover fence.
            removeGeoNotification(geo["id"].stringValue)
            return
        }

        //Data inicial da campanha < hoje, continua e dispara notificação
        guard notInitialDate <= Date() else {
            //Se não, campanha futura, não disparar notificação.
            return
        }
